...
static int Reset_Sensor_Regs(void);
...
extern int OV2640_Init(void)
{
  ...
  /* Check the I2C communications to the camera */
  if (SCCB_Read(REG_PID) != OV2640_PID)
  {
    LED_RED_ON();
    //BREAK();
  }
...
int Sensor_Snapshot(uint8_t *image)
{
  ...
  /* Start the DCMI */
  HAL_DCMI_Start_DMA(&hdcmi, DCMI_MODE_SNAPSHOT, addr, length);
