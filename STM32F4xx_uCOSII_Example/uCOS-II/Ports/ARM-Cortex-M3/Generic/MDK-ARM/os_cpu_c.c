...
/**
  * @brief  This function is called by OSInit() at the end of OSInit().
  * @param  None
  * @retval None
  */
...
/**
  * @brief  This function is called every second by uC/OS-II's statistics task.  
...
/**
  * @brief  This function is called by either OSTaskCreate() or OSTaskCreateExt() 
  *         to initialize the stack frame of the task being created.  This function 
...
#if USE_FPU    
    if ((opt & OS_TASK_OPT_SAVE_FP) != 0){
        for (i = 0u; i < 16; i++) {                  /* Initialize general-purpose Floating point registers */
...
/**
  * @brief  This function is called by OS_TCBInit() after setting up most of the TCB.
...
/**
  * @brief  Handle the system tick (SysTick) interrupt, which is used 
  *         to generate the uC/OS-II tick interrupt.
  * @param  None
  * @retval None
  */
